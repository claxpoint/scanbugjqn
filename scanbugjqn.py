...
#                                  
#
# Author     : foyhere, JQn , NAJA Police
# Tool       : BugJQn2022 : Scan Edition - IRN Government top secret plugin and service
# Usage      : python3 scanbugjqn.py "LINK"
# Description: SCAN-sys
#
#
...
# najabugjqn Help Context
def helper():
        print(bcolors.OKBLUE+"Information:"+bcolors.ENDC)
        print("------------")
        print("\t./bugjqn.py example.com: Scans the domain example.com.")
        print("\t./scanbugjqn.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./scanbugjqn.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./scanbugjqn.py --update   : Updates the scanner to the latest version.")
        print("\t./scanbugjqn.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits scanbugjqn.")
...
def logo():
    ...
    logo_ascii = """
    JQnService Terminal= run:scanbugjqn.py -ON- JQnService -NXT- T@ -NXT- @JQn;
    ShupXQL read;DONE: =
                                بِسْمِ ٱللَّٰهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ
        ╔══╗────────╔╦═══╗
        ║╔╗║────────║║╔═╗║
        ...
        ╚═══╩══╩═╗╠══╩══╗╠╝╚╝
        ───────╔═╝║─────╚╝
        ───────╚══╝  """+bcolors.BADFAIL+" ●"+bcolors.WARNING+"""_/(  _ _
                                
                     """+bcolors.ENDC+"""( Web Vulnerability JQn-sys Scanner)

                     ScanBugJQn By foyhere. """+bcolors.BG_LOW_TXT+"""Official Website:"""+bcolors.ENDC+""" https://foyhere.blog.ir - https://github.com/foyhere
    """
    print(logo_ascii)
    print(bcolors.ENDC)
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"NAJA received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/n_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/najabugjqn_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/scanbugjqn_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/scanbugjqn_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/scanbugjqn_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o scanbugjqn_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum scanbugjqn.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/skavngr/scanbugjqn/master/bbscanbugjqn.py -O scanbugjqn.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"v-UPDATED" + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"v-UPDATED@" +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"BugJQn Scanner was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"sys checked yours PLUGINonSYS: -HAVE?- true."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. ScanBugJQn will still perform the rest of the tests. Install these tools to fully utilize the functionality of RapidScan."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/scanbugjqn_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit scanbugjqn.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open("RS-Vulnerability-Report", "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/scanbugjqn_temp_"+vuln_info[arg1]
    ...
    os.system('rm /tmp/scanbugjqn_te* > /dev/null 2>&1') # Clearing previous scan files
